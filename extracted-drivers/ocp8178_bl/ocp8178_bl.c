...
static void entry_1wire_mode(struct ocp8178_backlight *gbl)
{
	...
	/* Explicitly set GPIO direction to output (fixes RP1 pinctrl issue) */
	gpiod_direction_output(gbl->gpiod, 0);
	
	/* First, ensure chip is in shutdown by holding low for 3+ seconds */
	gpiod_set_value(gbl->gpiod, 0);
	mdelay(SHUTDOWN_TIME); /* 3000ms = 3 seconds for shutdown */
	
	/* Now enter 1-wire mode with precise timing (interrupts disabled) */
